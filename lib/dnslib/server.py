...
import binascii,socket,struct,threading,time,logging
...
class DNSLogger:
    ...
    def log_recv(self,handler,data):
        logging.debug("%sReceived: [%s:%d] (%s) <%d> : %s" % (
    ...
    def log_request(self,handler,request):
        logging.debug("%sRequest: [%s:%d] (%s) / '%s' (%s)" % (
                    self.log_prefix(handler),
                    handler.client_address[0],
                    handler.client_address[1],
                    handler.protocol,
    ...
    def log_reply(self,handler,reply):
        if reply.header.rcode == RCODE.NOERROR:
            logging.debug("%sReply: [%s:%d] (%s) / '%s' (%s) / RRs: %s" % (
                    self.log_prefix(handler),
                    handler.client_address[0],
                    handler.client_address[1],
                    handler.protocol,
            ...
            logging.debug("%sReply: [%s:%d] (%s) / '%s' (%s) / %s" % (
                    self.log_prefix(handler),
                    handler.client_address[0],
                    handler.client_address[1],
                    handler.protocol,
    ...
    def log_truncated(self,handler,reply):
        logging.debug("%sTruncated Reply: [%s:%d] (%s) / '%s' (%s) / RRs: %s" % (
                    self.log_prefix(handler),
                    handler.client_address[0],
                    handler.client_address[1],
                    handler.protocol,
    ...
    def log_error(self,handler,e):
        logging.debug("%sInvalid Request: [%s:%d] (%s) :: %s" % (
                    self.log_prefix(handler),
                    handler.client_address[0],
                    handler.client_address[1],
                    handler.protocol,
